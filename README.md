...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create VictorResume-e804
  fly apps create VictorResume-e804-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app VictorResume-e804
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app VictorResume-e804-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'VictorResume-e804-staging' does not match app name in config file 'VictorResume-e804'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name VictorResume-e804-db
  fly postgres attach --app VictorResume-e804 VictorResume-e804-db

  fly postgres create --name VictorResume-e804-staging-db
  fly postgres attach --app VictorResume-e804-staging VictorResume-e804-staging-db
  ```
